...
const Network = layers => {
    ...
    const updateMiniBatch = (miniBatch, learningRate, lambda, trainingDataLength) => {
        ...
        for (let dataIndex = 0; dataIndex < miniBatch.length; ++dataIndex) {
            ...
            const { deltaWeights, deltaBiases } = backPropogation(data.inputs, data.outputs);
    ...
    const predict = data => {
        return data.map(data => {
            const output = feedForward(data.inputs).valueOf();
            const actual = output.indexOf(Math.max(...output)) + 1;
            const ideal = data.outputs.indexOf(1) + 1;
            return {
                actual,
                ideal
            }

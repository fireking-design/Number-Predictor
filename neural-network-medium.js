...
const mnistDataGeneration = async () => {
    ...
    data.map(data => {
        data = data.split(',');
        const output = math.zeros([10])
        output[data.shift()] = 1
        let input = data.map(x => (+x) / 255);
...
const Network = layers => {
    ...
    const costDirivative = (outputs, ideal) => math.subtract(math.flatten(outputs), ideal);
    ...
    const backPropogation = (inputs, ideal) => {
        ...
        let deltaVector = costDirivative(activations[activations.length - 1], ideal);
    ...
    const train = (trainingData, epochs, miniBatchSize, learningRate, lambda, testingData) => {
        ...
        for (let epochIndex = 0; epochIndex < epochs; ++epochIndex) {
            ...
            writeFileSync('client/data/weights.json', JSON.stringify({ weights, biases }));
        }
    }
...
mnistDataGeneration().then(value => {
    const { train } = Network([784, 107, 10]);
    const epochs = 30;
    const miniBatchSize = 100;
    const learningRate = 0.5;
    const lambda = 5;
    train(value.train, epochs, miniBatchSize, learningRate, lambda, value.test);
});